...
object Project {
    internal fun CreateTree(root: Word) {                  //어플 실행 전 트리를 구축하는 함수
        ...
        //3층
        val level2_word1_word1_word1 = Word("if문", "1. \"제어문\" 이란 무엇인가?\n" +
                " \n" +
        ...
        val level2_word1_word1_word2 = Word("for문", "컴퓨터 프로그래밍에서 for 루프는 반복문의 일종으로, 특정한 부분의 코드가 반복적으로 수행될 수 있도록 한다. for 루프는 while 루프와 같은 다른 반복문과는 달리 일반적으로 해당 루프에 연계된 루프 변수가 존재하며, 그 변수의 비교 및 증감을 위해서 별도의 문법을 할애한 점이 구분된다.\n" +
                "\n" +
                "for 루프의 이름은 영어 표현 “For …, do …”(…동안 …를 하라)에서 유래하였다. 포트란과 같은 언어에서는 for 대신 do 예약어를 사용하며 그 문법의 이름도 do 루프이지만, 이를 제외하고는 for 루프와 큰 차이가 없다.", level2_word1_word1)
    ...
    //        Scanner sc = new Scanner (System.in);             //원래는  이름, 설명 받아서 새로운 Node만들도록
    //        Word Root = new Word();
